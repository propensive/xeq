...
// Produces a self-contained per-platform executable by patching a bare runner
// stub's ETHRCFG block (build id, Java version policy, ML-DSA-44 public key) and
// appending the application JAR at EOF — the build side of `spec/ethrcfg.md`.
//
// A daemon implementation may carry its own builder for its self-packaging path
// (Soundness's `ethereal.Assembler` does), which is why the format is specified
// rather than shared: the two implementations are checked against each other by
// the tests, not by a dependency.
object Assembler:
  // v2 ETHRCFG layout — specified in `spec/ethrcfg.md`, read by `src/runner/src/config.rs`.
