// Marker-patched static data. A builder scans for the magic `ETHRCFG\x02` and overwrites
// the bytes that follow. Specified in spec/ethrcfg.md, which is what a builder outside this
// repository (such as Soundness's `ethereal.Assembler`) is written against.
//
